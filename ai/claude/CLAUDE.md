...
- Never use cat/grep/find/sed/ls. Use bat/rg/fd/sd/eza instead. Install via brew if missing. (Intentional exception: `settings.json` permits these classic tools as a gated fallback for the rare case where no modern equivalent fits; `sed -i` is denied so in-place overwrites still prompt. The preference above is behavioral and always wins by default.)
...
> **Runtime-specific (Claude Code) — intentional, NOT drift.** Claude Code sub-agents receive skill **paths** and read the full `SKILL.md` files themselves, preserving author intent. The OpenCode runtime (`~/.config/opencode/AGENTS.md`) deliberately differs: it injects compact rule **text** because its sub-agents do not read files. Do not "unify" these two flows — each is correct for its runtime.
