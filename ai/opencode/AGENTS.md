...
> **Runtime-specific (OpenCode) — intentional, NOT drift.** OpenCode sub-agents receive compact rule **text** pre-resolved by the orchestrator because they do not read `SKILL.md` files. The Claude Code runtime (`~/.claude/CLAUDE.md`) deliberately differs: it passes skill **paths** and its sub-agents read the files directly. Do not "unify" these two flows — each is correct for its runtime.
